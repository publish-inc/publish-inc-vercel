...
function debugGetAllUsers() {
  ...
  var pageParam = getPublicPageParam_(e);
  if (String(pageParam || '').toLowerCase() === 'fd') pageParam = 'final-data';
  if (String(pageParam || '').toLowerCase() === 'author') pageParam = 'final-data';
  var publicCode = (e && e.parameter && (e.parameter.code || e.parameter.c)) || '';
  var publicToken = (e && e.parameter && (e.parameter.token || e.parameter.t)) || '';
  if (String(pageParam || '').toLowerCase() === 'customer-tracking' || String(pageParam || '').toLowerCase() === 'tracking') {
    var trackingUrl = ScriptApp.getService().getUrl().replace(/\/dev$/, '/exec');
    var trackingTemplate = HtmlService.createTemplateFromFile('PublicTracking');
    trackingTemplate.webAppUrl = trackingUrl;
    return trackingTemplate
      .evaluate()
      .addMetaTag('viewport', 'width=device-width, initial-scale=1')
      .setTitle('Tracking Naskah')
      .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  }
  ...
  return template
    .evaluate()
    .addMetaTag('viewport', 'width=device-width, initial-scale=1')
    .setTitle('Publish Inc. Portal KPI')
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
...
function getPublicPageParam_(e) {
  var pageParam = (e && e.parameter && (e.parameter.page || e.parameter.p)) || '';
  if (pageParam) return pageParam;
  var query = e && e.queryString ? String(e.queryString || '').trim() : '';
  var shorthand = query.match(/^=(author|customer-tracking|tracking|final-data|fd)$/i);
...
// ==========================================
var PORTAL_DATABASE_ID = '181QHJCkaAmLBUsHLvJBQGeJj_QxdAe0zbxnf3YVEyb8';
...
var H = {
  ...
  DeadlineSettings: ['id', 'penerbit', 'paket', 'totalHari', 'administrasi', 'administrasiMulai', 'administrasiSelesai', 'proofreading', 'proofreadingMulai', 'proofreadingSelesai', 'layout', 'layoutMulai', 'layoutSelesai', 'isbn', 'produksi', 'produksiMulai', 'produksiSelesai', 'distribusi', 'distribusiMulai', 'distribusiSelesai', 'selesai', 'selesaiMulai', 'selesaiSelesai', 'updatedAt'],
...
function saveHrdUserFromManagement(p) {
  p = p || {};
  var oldUsername = String(p.old_username || '').trim();
  var nextUsername = String(p.username || '').trim();
  var users = getAllData('Users', H.Users);
  for (var i = 0; i < users.length; i++) {
    var userName = String(users[i].username || '').trim();
    var role = normalizeRole_(users[i].role);
    if (oldUsername && userName === oldUsername && role !== 'hrd') {
      return { success: false, message: 'Akses ini hanya boleh mengubah akun HRD.' };
    }
    if (nextUsername && userName === nextUsername && (!oldUsername || userName !== oldUsername) && role !== 'hrd') {
      return { success: false, message: 'Username sudah dipakai oleh akun non-HRD.' };
    }
  }
  return saveUserData({
    old_username: oldUsername,
    employeeId: p.employeeId || 'EMP-HRD',
    username: nextUsername,
    password: p.password || '',
    nama: p.nama || 'HRD',
    role: 'hrd',
    profileComplete: '1',
    statusKaryawan: p.statusKaryawan || 'Karyawan Tetap',
    departemen: p.departemen || 'HRD',
    jabatan: p.jabatan || 'HRD',
    email: p.email || '',
    telepon: p.telepon || '',
    alamat: p.alamat || '',
...
function getDeadlineTimelineStages_() {
  return ['administrasi', 'proofreading', 'layout', 'produksi', 'distribusi', 'selesai'];
}

function normalizeDeadlineSetting_(row) {
  row = row || {};
  var normalized = Object.assign({}, row);
  var stages = getDeadlineTimelineStages_();
  var previousEnd = 0;
  stages.forEach(function(key) {
    var startKey = key + 'Mulai';
    var endKey = key + 'Selesai';
    var legacyEnd = Math.max(0, Number(normalized[key] || 0));
    var end = Math.max(0, Number(normalized[endKey] || legacyEnd || previousEnd));
    var start = Math.max(0, Number(normalized[startKey] !== undefined && normalized[startKey] !== '' ? normalized[startKey] : previousEnd));
...
function getDeadlineSettings() {
  ...
  rows = ensureGlobalAccelerationDeadlineSettings_(rows);
  var allowedPackages = {};
  getAllData('SettingsPub', H.SettingsPub).forEach(function(pub) {
    var packages = normalizePenerbitPackages_(pub.paket);
    packages.forEach(function(paket) {
      var key = String((paket && paket.nama) || paket || '').trim().toLowerCase();
      if (key) allowedPackages[key] = true;
    });
  });
  allowedPackages.vip = true;
  allowedPackages.vvip = true;
  if (Object.keys(allowedPackages).length) {
    rows = rows.filter(function(row) {
      return allowedPackages[String(row.paket || '').trim().toLowerCase()];
    });
  }
  return { success: true, data: rows.map(normalizeDeadlineSetting_) };
}

function defaultAccelerationDeadlineSetting_(type) {
  var isVvip = String(type || '').toUpperCase() === 'VVIP';
  return isVvip ? {
    id: 'DL-VVIP', penerbit: '', paket: 'VVIP', totalHari: 14,
    administrasi: 1, administrasiMulai: 0, administrasiSelesai: 1,
    proofreading: 2, proofreadingMulai: 1, proofreadingSelesai: 2,
    layout: 4, layoutMulai: 2, layoutSelesai: 4,
    produksi: 10, produksiMulai: 4, produksiSelesai: 10,
    distribusi: 12, distribusiMulai: 10, distribusiSelesai: 12,
    selesai: 14, selesaiMulai: 12, selesaiSelesai: 14,
    updatedAt: Utilities.formatDate(new Date(), getDBTimezone(), 'yyyy-MM-dd')
  } : {
    id: 'DL-VIP', penerbit: '', paket: 'VIP', totalHari: 21,
    administrasi: 2, administrasiMulai: 0, administrasiSelesai: 2,
    proofreading: 4, proofreadingMulai: 2, proofreadingSelesai: 4,
    layout: 7, layoutMulai: 4, layoutSelesai: 7,
    produksi: 15, produksiMulai: 7, produksiSelesai: 15,
    distribusi: 18, distribusiMulai: 15, distribusiSelesai: 18,
    selesai: 21, selesaiMulai: 18, selesaiSelesai: 21,
    updatedAt: Utilities.formatDate(new Date(), getDBTimezone(), 'yyyy-MM-dd')
  };
}

function ensureGlobalAccelerationDeadlineSettings_(rows) {
  rows = rows || [];
  ['VIP', 'VVIP'].forEach(function(type) {
    var hasGlobal = rows.some(function(row) {
      return String(row.paket || '').trim().toUpperCase() === type && !String(row.penerbit || '').trim();
    });
    if (hasGlobal) return;
    var existingAny = rows.filter(function(row) {
      return String(row.paket || '').trim().toUpperCase() === type;
    })[0];
    var seed = existingAny ? Object.assign({}, existingAny, { id: 'DL-' + type, penerbit: '', paket: type }) : defaultAccelerationDeadlineSetting_(type);
    saveData('DeadlineSettings', H.DeadlineSettings, seed, 'id');
    rows.push(seed);
  });
  return rows;
}
...
function saveDeadlineSetting(p) {
  ...
  p.totalHari = Math.max(0, Number(p.totalHari || 0));
  getDeadlineTimelineStages_().forEach(function(key) {
    p[key + 'Mulai'] = Math.max(0, Number(p[key + 'Mulai'] || 0));
    p[key + 'Selesai'] = Math.max(0, Number(p[key + 'Selesai'] || p[key] || 0));
    p[key] = p[key + 'Selesai'];
  });
  if (!p.totalHari) p.totalHari = p.selesaiSelesai || p.selesai;
  p.selesaiSelesai = p.totalHari;
  p.selesai = p.totalHari;
  p.isbn = '';
  p = normalizeDeadlineSetting_(p);
...
function resolveDeadlinePlan_(penerbit, paket, percepatanProses) {
  ...
  if (accelerationDays) {
    ...
    if (normAcceleration === 'VIP' || normAcceleration === 'VVIP') {
      var publisherAccelerationPlan = findDeadlinePlan_(penerbit, normAcceleration);
      if (publisherAccelerationPlan) return publisherAccelerationPlan;
      var globalAccelerationPlan = findDeadlinePlan_('', normAcceleration);
      if (globalAccelerationPlan) return globalAccelerationPlan;
    }
...
function buildDeadlinePlan_(startDate, penerbit, paket, overrideDeadline, percepatanProses) {
  var rawPlan = resolveDeadlinePlan_(penerbit, paket, percepatanProses);
  if (!rawPlan) return null;
  var plan = normalizeDeadlineSetting_(rawPlan);
  ...
  var dayFor = function(key) {
    var raw = Math.max(0, Number(plan[key + 'Selesai'] || plan[key] || 0));
    if (key === 'selesai') raw = Number(plan.selesaiSelesai || plan.selesai || total);
    return Math.max(0, Math.round(raw * ratio));
  };
  var startDayFor = function(key) {
    return Math.max(0, Math.round(Math.max(0, Number(plan[key + 'Mulai'] || 0)) * ratio));
  };
  var base = {
  ...
  var baseRanges = {};
  getDeadlineTimelineStages_().forEach(function(key) {
    var endDays = key === 'selesai' ? finalTotal : dayFor(key);
    baseRanges[key] = {
      start: addBusinessDays_(startDate, startDayFor(key), holidayDates),
      end: addBusinessDays_(startDate, endDays, holidayDates)
    };
  });
  return {
    settingId: plan.id,
    ...
    actualRanges: Object.assign({}, baseRanges)
  };
}

function workflowDateOnly_(value) {
  return String(value || '').substring(0, 10);
}

function buildTimelineRangesFromDeadlines_(startDate, deadlines, skipProofreading) {
  deadlines = deadlines && typeof deadlines === 'object' ? deadlines : {};
  var start = workflowDateOnly_(startDate) || workflowDateOnly_(deadlines.administrasi);
  var adminEnd = workflowDateOnly_(deadlines.administrasi) || start;
  var proofEnd = skipProofreading ? '' : workflowDateOnly_(deadlines.proofreading);
  var layoutStart = skipProofreading ? adminEnd : (proofEnd || adminEnd);
  var layoutEnd = workflowDateOnly_(deadlines.layout) || layoutStart;
  var productionStart = layoutEnd;
  var productionEnd = workflowDateOnly_(deadlines.produksi) || productionStart;
  var distributionStart = productionEnd;
  var distributionEnd = workflowDateOnly_(deadlines.distribusi || deadlines.selesai) || distributionStart;
  var doneEnd = workflowDateOnly_(deadlines.selesai) || distributionEnd;
  return {
    administrasi: { start: start, end: adminEnd },
    proofreading: skipProofreading ? { start: '', end: '' } : { start: adminEnd, end: proofEnd || adminEnd },
    layout: { start: layoutStart, end: layoutEnd },
    produksi: { start: productionStart, end: productionEnd },
    distribusi: { start: distributionStart, end: distributionEnd },
    selesai: { start: distributionEnd, end: doneEnd }
  };
}

function workflowStageLabel_(stageKey) {
  var labels = {
    administrasi: 'Administrasi',
    pic_editor: 'Proofreading',
    editor: 'Proofreading',
    pic_layouter: 'Desain Cover & Layout',
    layouter: 'Desain Cover & Layout',
    produksi: 'Produksi',
    distribusi: 'Distribusi',
    selesai: 'Selesai'
  };
  return labels[String(stageKey || '').trim().toLowerCase()] || 'Administrasi';
}

function isWorkflowDoneStatus_(status) {
  return ['done', 'complete', 'selesai', 'terbit'].indexOf(String(status || '').trim().toLowerCase()) !== -1;
}

function inferWorkflowStage_(n) {
  n = n || {};
  if (String(n.statusProses || '').trim().toLowerCase() === 'dibatalkan') return 'dibatalkan';
  if (isWorkflowDoneStatus_(n.statusDistribusi) || trackingStageKey_(n.timeline) === 'selesai') return 'selesai';
  if (isWorkflowDoneStatus_(n.statusProduksi) || String(n.statusDistribusi || '').trim().toLowerCase() !== 'menunggu' && String(n.statusDistribusi || '').trim()) return 'distribusi';
  if (isWorkflowDoneStatus_(n.statusLayouter)) return 'produksi';
  if (String(n.picLayouter || '').trim()) return 'layouter';
  if (isWorkflowDoneStatus_(n.statusEditor) || isProofreadingSkipped_(n.butuhProofreading)) return 'pic_layouter';
  if (String(n.picEditor || '').trim()) return 'editor';
  if (String(n.productionStartedAt || '').trim()) return 'pic_editor';
  return 'administrasi';
}

function workflowStageRank_(stageKey) {
  var order = {
    administrasi: 1,
    pic_editor: 2,
    editor: 3,
  ...
  return order[String(stageKey || '').trim().toLowerCase()] || 0;
}

function effectiveWorkflowStageFromStatuses_(n) {
  n = n || {};
  var inferred = inferWorkflowStage_(n);
  var intake = String(n.intakeStatus || '').trim().toLowerCase();
  var statusProduksi = String(n.statusProduksi || '').trim().toLowerCase();
  var statusDistribusi = String(n.statusDistribusi || '').trim().toLowerCase();
  var hasProductionSignal = !!String(n.productionStartedAt || '').trim() ||
    isWorkflowDoneStatus_(n.statusLayouter) ||
    isWorkflowDoneStatus_(n.statusProduksi) ||
    isWorkflowDoneStatus_(n.statusDistribusi) ||
    (statusProduksi && statusProduksi !== 'menunggu') ||
    (statusDistribusi && statusDistribusi !== 'menunggu');
  if (['waiting_final_data', 'waiting_admin', 'admin_done'].indexOf(intake) !== -1 && !hasProductionSignal) {
    inferred = 'administrasi';
  }
  return inferred;
}

function syncWorkflowStageFromStatuses_(n, actor) {
  n = n || {};
  var inferred = effectiveWorkflowStageFromStatuses_(n);
  var current = String(n.workflowStage || '').trim().toLowerCase();
  if (!current || workflowStageRank_(inferred) > workflowStageRank_(current)) {
    setWorkflowStage_(n, inferred, actor || n.workflowOwnerUsername || n.assignedCssUsername || '');
  }
  if (inferred === 'produksi' && String(n.statusProses || '').trim().toLowerCase() !== 'selesai') {
    n.statusProses = 'Produksi';
  } else if (inferred === 'distribusi') {
    n.statusProses = 'Distribusi';
  } else if (inferred === 'selesai') {
    n.statusProses = 'Selesai';
  }
  return n;
}

function setWorkflowStage_(n, stageKey, actor) {
  stageKey = String(stageKey || '').trim().toLowerCase();
  n.workflowStage = stageKey;
  n.timeline = workflowStageLabel_(stageKey);
  n.stageStartedAt = nowDateTime_();
  if (actor) {
    var user = findPortalUserByUsername_(actor) || {};
    n.workflowOwnerUsername = user.username || actor || '';
    n.workflowOwnerName = user.nama || actor || '';
  }
  return n;
}

function refreshNaskahTimelineRanges_(n) {
  n = n || {};
  ...
  if (n.baseDeadlines && typeof n.baseDeadlines === 'object') {
    n.baseTimelineRanges = buildTimelineRangesFromDeadlines_(startDate, n.baseDeadlines, skipProofreading);
  }
  if (n.deadlines && typeof n.deadlines === 'object') {
    n.timelineRanges = buildTimelineRangesFromDeadlines_(startDate, n.deadlines, skipProofreading);
  } else if (n.baseTimelineRanges) {
    n.timelineRanges = n.baseTimelineRanges;
  }
  return n;
}
...
function syncNaskahDeadlinesFromSettings() {
  ...
  var keys = getDeadlineTimelineStages_();
  var updated = 0;
  var skipped = 0;
  var noPlan = 0;
  ...
  rows.forEach(function(n) {
    ...
    refreshNaskahTimelineRanges_(n);
    if (!skipProofreading && plan.baseRanges) {
      n.baseTimelineRanges = plan.baseRanges;
      n.timelineRanges = plan.actualRanges || plan.baseRanges;
    }
    n.deadlinePlan = {
...
function syncNaskahTimelineFromStatuses_(n) {
  n = n || {};
  var steps = ['Praproduksi', 'Administrasi', 'Proofreading', 'Desain Cover & Layout', 'Produksi', 'Distribusi', 'Selesai'];
  ...
  if (isPicProductionStageComplete_(n.statusLayouter)) moveTo('Produksi');
...
function isNaskahProofreadingSkipped_(n) {
  ...
  if (editorDone && ['layout', 'produksi', 'distribusi', 'selesai'].indexOf(stageKey) !== -1) return true;
  return false;
}
...
function applyTrackingDelays_(baseDeadlines, delays) {
  var steps = ['administrasi','proofreading','layout','produksi','distribusi','selesai'];
...
function saveNaskahTrackingDelay(id, stageKey, days, reasonCode, note) {
  ...
  for (var i = 0; i < rows.length; i++) {
    if (rows[i].id === id) {
      ...
      refreshNaskahTimelineRanges_(rows[i]);
...
function getDeadlineAlerts(username, role) {
  role = normalizeRole_(role || '');
  return { success: true, data: [], today: Utilities.formatDate(new Date(), getDBTimezone(), 'yyyy-MM-dd'), disabled: true };
  var user = findPortalUserByUsername_(username) || {};
...
function completeDeadlineAlertStage(naskahId, stageKey, username, role) {
  role = normalizeRole_(role || '');
  stageKey = trackingStageKey_(stageKey);
  if (role !== 'css' && role !== 'manajemen') return { success: false, message: 'Aksi selesai hanya untuk CCO atau manajemen.' };
  var rows = getAllData('Naskah', H.Naskah);
  for (var i = 0; i < rows.length; i++) {
    if (String(rows[i].id || '') !== String(naskahId || '')) continue;
    var n = rows[i];
    if (role === 'css' && !isCssOwner_(n, username)) return { success: false, message: 'Naskah ini bukan tanggung jawab CCO Anda.' };
    if (stageKey === 'administrasi') {
      if (role !== 'css') return { success: false, message: 'Mulai produksi administrasi hanya bisa dilakukan oleh CCO penanggung jawab.' };
...
function completeIsbnSubmissionAlert(naskahId, username, role) {
  role = normalizeRole_(role || '');
  if (role !== 'css' && role !== 'manajemen') return { success: false, message: 'Aksi pengajuan ISBN hanya untuk CCO atau manajemen.' };
  var rows = getAllData('Naskah', H.Naskah);
  for (var i = 0; i < rows.length; i++) {
    if (String(rows[i].id || '') !== String(naskahId || '')) continue;
    var n = rows[i];
...
function updateDeadlineAlertIsbnStatus(naskahId, status, username, role) {
  role = normalizeRole_(role || '');
  if (role !== 'css' && role !== 'manajemen') return { success: false, message: 'Aksi ISBN hanya untuk CCO atau manajemen.' };
...
function getPublicTrackingData(code, phone4) {
  ...
  var baseRanges = n.baseTimelineRanges && typeof n.baseTimelineRanges === 'object'
    ? n.baseTimelineRanges
    : buildTimelineRangesFromDeadlines_(n.tanggal, base, skipProofreading);
  var actualRanges = buildTimelineRangesFromDeadlines_(n.tanggal, actual, skipProofreading);
  var steps = [
  ...
  var statusDone = function(status) {
    return status === 'done' || status === 'complete' || status === 'selesai' || status === 'terbit';
  };
  ...
  if (statusDone(layouterStatus)) currentIdx = Math.max(currentIdx, stepKeys.indexOf('produksi'));
  ...
  var timeline = steps.map(function(step, idx) {
    ...
    return {
      ...
      rangeAwal: baseRanges[step.key] || { start: '', end: '' },
      rangeTerbaru: isSkippedProofreading ? { start: '', end: '' } : (actualRanges[step.key] || baseRanges[step.key] || { start: '', end: '' }),
      delayDays: delayDays,
      reason: delayReason,
...
function saveDealData(p) {
  ...
  if (p.percepatanProses !== 'VIP' && p.percepatanProses !== 'VVIP') {
    p.percepatanProses = '';
  }
  ...
  if (!selectedCss && isNew && String(p.source || '').toLowerCase() !== 'legacy') {
    return { success: false, message: 'CCO penanggung jawab wajib dipilih sebelum deal disimpan.' };
  }
  ...
  if (isNew && p.source !== 'legacy') {
    ...
    var baseTimelineRanges = skipProofreading ? buildTimelineRangesFromDeadlines_(p.tanggal, baseDeadlines, skipProofreading) : (planForNaskah ? planForNaskah.baseRanges : buildTimelineRangesFromDeadlines_(p.tanggal, baseDeadlines, skipProofreading));
    var timelineRanges = skipProofreading ? buildTimelineRangesFromDeadlines_(p.tanggal, actualDeadlines, skipProofreading) : (planForNaskah ? planForNaskah.actualRanges : buildTimelineRangesFromDeadlines_(p.tanggal, actualDeadlines, skipProofreading));
    var newNaskah = {
      ...
      statusEditor: skipProofreading ? 'Dilewati' : 'Menunggu', statusLayouter: 'Menunggu', statusProduksi: 'Menunggu', statusDistribusi: 'Menunggu', deadlines: actualDeadlines, catatanCS: p.catatan || '',
      statusProses: p.statusProses || 'Belum Siap Proses',
      source: 'cs',
      butuhProofreading: p.butuhProofreading,
      trackingCode: p.trackingCode,
      baseDeadlines: baseDeadlines,
      baseTimelineRanges: baseTimelineRanges,
      timelineRanges: timelineRanges,
      trackingDelays: {},
      deadlinePlan: p.deadlinePlan || '',
      intakeStatus: 'waiting_final_data',
      workflowStage: 'administrasi',
      workflowOwnerUsername: selectedCss.username || '',
      workflowOwnerName: selectedCss.nama || '',
      stageStartedAt: nowDateTime_(),
    ...
    for(var i=0; i<naskahs.length; i++) {
       if (naskahs[i].dealId === p.id) {
           ...
           if (p.butuhProofreading === '0') {
             n.picEditor = '';
             n.statusEditor = 'Dilewati';
             if (n.deadlines && typeof n.deadlines === 'object') n.deadlines.proofreading = '';
             var timelineOrder = ['Administrasi', 'Proofreading', 'Desain Cover & Layout', 'Produksi', 'Distribusi', 'Selesai'];
           ...
           } else if (oldButuhProofreading === '0' && ['Complete', 'Dilewati'].indexOf(String(n.statusEditor || '')) !== -1 && !n.picEditor) {
...
function getSettingsData() { 
  ...
  if(pubs.length > 0) {
      pubs.forEach(function(p){ 
          ...
          p.paket = normalizePenerbitPackages_(p.paket);
      });
  }
...
function normalizePackageColor_(value, idx) {
  var defaults = ['#FF6B35', '#2563eb', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#14b8a6'];
  var color = String(value || '').trim();
  if (!/^#[0-9a-fA-F]{6}$/.test(color)) color = defaults[Math.abs(Number(idx || 0)) % defaults.length];
  return color;
}

function normalizePenerbitPackages_(packages) {
  if (typeof packages === 'string') {
    try { packages = JSON.parse(packages); } catch(e) { packages = packages.split(','); }
  }
  ...
  packages.forEach(function(pkg, idx) {
    ...
    if (pkg && typeof pkg === 'object') {
      nama = String(pkg.nama || pkg.name || pkg.label || '').trim();
      warna = pkg.warna || pkg.color || '';
    } else {
      nama = String(pkg || '').trim();
    }
    if (!nama) return;
    var key = nama.toLowerCase();
    if (seen[key]) return;
    seen[key] = true;
    result.push({ nama: nama, warna: normalizePackageColor_(warna, idx) });
  });
  return result;
}

function getPublishLogoDataUri_() {
  try {
    var html = HtmlService.createHtmlOutputFromFile('JS_Core').getContent();
    var match = html.match(/data:image\/png;base64,[A-Za-z0-9+/=]+/);
...
function saveCetakJob(p, username) {
  ...
  if (!p.judul || !p.tanggal || !p.ukuran || !p.warna || !p.jenisKertas || !p.cover || !p.jumlahEks || !p.nominal) {
    return { success: false, message: 'Tanggal, judul, spesifikasi cetak, jumlah eks, dan nominal wajib diisi.' };
  }
  ...
  return { success: true, data: { kpiId: p.kpiId, isNew: isNew, kpiOnly: true } };
}
  ...
  rows.forEach(function(n) {
    ...
    if (!n.baseDeadlines && n.paket) {
      ...
      if (plan) {
        ...
        n.baseTimelineRanges = plan.baseRanges;
        n.timelineRanges = plan.actualRanges;
        n.deadlinePlan = plan.settingId;
        shouldSave = true;
      }
    }
    var previousRanges = JSON.stringify(n.timelineRanges || {});
    refreshNaskahTimelineRanges_(n);
    if (JSON.stringify(n.timelineRanges || {}) !== previousRanges) shouldSave = true;
    if (!String(n.workflowStage || '').trim()) {
      n.workflowStage = inferWorkflowStage_(n);
      n.timeline = workflowStageLabel_(n.workflowStage);
      shouldSave = true;
    }
    if (shouldSave) {
      saveData('Naskah', H.Naskah, n, 'id');
...
function isNewNaskahIntake_(n) {
  var status = String(n.statusProses || '').trim().toLowerCase();
  var intake = String(n.intakeStatus || '').trim().toLowerCase();
  var workflowStage = effectiveWorkflowStageFromStatuses_(n);
...
function hasSubmittedAuthorFinalData_(n) {
  n = n || {};
  if (String(n.finalDataSubmittedAt || '').trim()) return true;
  var finalTitle = String(n.finalJudul || '').trim();
  var finalAuthor = String(n.finalPenulisUtama || '').trim();
  var finalAuthors = normalizeAuthorList_(n.finalPenulisList || []);
  return !!(finalAuthor || finalAuthors.length) && !!(finalTitle || String(n.judul || '').trim());
}

function selfHealAuthorFinalDataStatus_(n) {
  n = n || {};
  if (!hasSubmittedAuthorFinalData_(n)) return false;
  var changed = false;
  if (!String(n.finalDataSubmittedAt || '').trim()) {
    n.finalDataSubmittedAt = nowDateTime_();
    changed = true;
  }
  if (String(n.intakeStatus || '').trim().toLowerCase() === 'waiting_final_data') {
    n.intakeStatus = 'waiting_admin';
    changed = true;
  }
  if (String(n.statusProses || '').trim().toLowerCase() === 'belum siap proses') {
    n.statusProses = 'Administrasi Naskah';
    changed = true;
  }
  if (!String(n.timeline || '').trim() || trackingStageKey_(n.timeline) === 'praproduksi') {
...
function getCssNewNaskah(username) {
  ...
  var rows = getAllData('Naskah', H.Naskah);
  rows.forEach(function(n) {
    var beforeStage = String(n.workflowStage || '').trim();
    var beforeTimeline = String(n.timeline || '').trim();
    var beforeStatus = String(n.statusProses || '').trim();
    syncWorkflowStageFromStatuses_(n, username);
    if (beforeStage !== String(n.workflowStage || '').trim() ||
        beforeTimeline !== String(n.timeline || '').trim() ||
        beforeStatus !== String(n.statusProses || '').trim()) {
      saveData('Naskah', H.Naskah, n, 'id');
      changed = true;
    }
  });
  var data = rows.filter(function(n) {
    return (isNewNaskahIntake_(n) || isCssWorkflowNaskah_(n, username)) && isCssOwner_(n, username);
  }).map(function(n) {
    if (selfHealAuthorFinalDataStatus_(n)) {
      saveData('Naskah', H.Naskah, n, 'id');
      changed = true;
    }
    if (!n.finalDataToken) {
...
function getNaskahFinalDataForm(code, token) {
  ...
  if (n && selfHealAuthorFinalDataStatus_(n)) {
    saveData('Naskah', H.Naskah, n, 'id');
  }
  ...
  return {
    ...
    data: {
      ...
      ukuran: n.ukuran || '',
      noHpPenulis: n.noHpPenulis || '',
      pekerjaan: n.authorPekerjaan || '',
      ktp: n.authorKtp || '',
      alamat: n.authorAlamat || '',
      spkSettings: spkGetAdminSettings(),
      submitted: !!n.finalDataSubmittedAt
    }
  };
}

function verifyAuthorAccess(token, phone4) {
  token = String(token || '').trim();
  if (!token) return { success: false, message: 'Masukkan token author.' };
  var n = getAllData('Naskah', H.Naskah).find(function(row) {
    return String(row.finalDataToken || '').trim() === token;
  });
  if (!n || !isNewNaskahIntake_(n)) {
    return { success: false, message: 'Token author tidak valid atau sudah tidak aktif.' };
  }
  if (selfHealAuthorFinalDataStatus_(n)) {
    saveData('Naskah', H.Naskah, n, 'id');
  }
  ...
  if (!phoneCheck.success) {
    return { success: false, message: 'Token author atau verifikasi nomor HP tidak sesuai.' };
  }
  return getNaskahFinalDataForm('', token);
}
...
function submitNaskahFinalData(code, token, payload) {
  ...
  var finalJudul = String(payload.judul || n.finalJudul || n.judul || '').replace(/\s+/g, ' ').trim();
  var authors = normalizeAuthorList_(payload.penulisList || payload.penulis || n.finalPenulisList || n.finalPenulisUtama || n.penulis || []);
  var editors = normalizeAuthorList_(payload.editorList || payload.editors || []);
  var pekerjaan = spkNormalizeString_(payload.pekerjaan, 120);
  var ktp = String(payload.ktp || '').replace(/\D/g, '');
  var alamat = spkNormalizeString_(payload.alamat, 500);
  ...
  if (!pekerjaan) return { success: false, message: 'Pekerjaan penulis wajib diisi.' };
  if (!/^\d{16}$/.test(ktp)) return { success: false, message: 'NIK/KTP wajib 16 digit angka.' };
  if (!alamat) return { success: false, message: 'Alamat penulis wajib diisi.' };
  ...
  n.authorPekerjaan = pekerjaan;
  n.authorKtp = ktp;
  n.authorAlamat = alamat;
  n.judul = finalJudul;
...
function spkGetFinalTokenLog() {
  var rows = getAllData('Naskah', H.Naskah).filter(function(n) {
    if (!String(n.finalDataToken || '').trim()) return false;
    if (String(n.cancelApprovedAt || '')) return false;
    if (String(n.statusProses || '').toLowerCase() === 'dibatalkan') return false;
    return true;
  }).sort(function(a, b) {
    return String(b.tanggal || b.finalDataSubmittedAt || '').localeCompare(String(a.tanggal || a.finalDataSubmittedAt || ''));
  });
  return rows.map(spkMapNaskahFinalToken_);
}

function spkGetPdfAdministrasiList() {
  return spkGetFinalTokenLog().filter(function(row) {
    return !!String(row.submittedAt || '').trim();
  });
}

function spkMapNaskahFinalToken_(n) {
  var authors = normalizeAuthorList_(n.finalPenulisList || n.finalPenulisUtama || n.penulis || []);
  var editors = normalizeAuthorList_(n.finalEditorList || []);
  var submittedAt = String(n.finalDataSubmittedAt || '');
  var adminDone = !!String(n.adminDoneAt || '');
  return {
    id: n.id || '',
    token: n.finalDataToken || '',
    trackingCode: n.trackingCode || '',
    finalDataUrl: getPublicFinalDataUrl_(n),
    judul: n.finalJudul || n.judul || '',
    penulis: authors.length ? publicAuthorDisplay_(authors) : n.penulis || '',
    penulisList: authors,
    editorList: editors,
    editor: editors.join(', '),
    penerbit: n.penerbit || '',
    paket: n.paket || '',
    ukuran: n.ukuran || '',
    isbn: spkGetNaskahIsbnText_(n),
    hp: n.noHpPenulis || '',
    pekerjaan: n.authorPekerjaan || '',
    ktp: n.authorKtp || '',
    alamat: n.authorAlamat || '',
    submittedAt: submittedAt,
    status: adminDone ? 'ADMIN_DONE' : submittedAt ? 'SUBMITTED' : 'WAITING_AUTHOR',
    adminDoneAt: n.adminDoneAt || ''
  };
}

var SPK_ADMIN_DOC_TEMPLATES = {
  sktt: '1mSQTofjnRRffI2DMA9c4ep97YbTcQkBznPiEN2JICUA',
  loa: '1t96X19bu1ameuTTsdoNQnt6HwHDyz9uLy3T8QVj8Ag0',
  keaslian: '1A9-6q_puexXHJ83gT-m4ic3FfNi-NkHKJJxvOkn5BpQ',
  isbn: '1QfUDL0awOPVKM4wcfj4u5PkyWUdYY4_jPF2QCEBqsOU'
};

function spkGenerateAdminTemplatePDF(token, type, options) {
  type = String(type || '').toLowerCase();
  options = options || {};
  var templateId = SPK_ADMIN_DOC_TEMPLATES[type];
  if (!templateId) return { success: false, message: 'Jenis dokumen tidak dikenali.' };
  var n = getAllData('Naskah', H.Naskah).find(function(row) {
    return String(row.finalDataToken || '').trim() === String(token || '').trim();
  });
  if (!n) return { success: false, message: 'Data naskah tidak ditemukan.' };

  if (type === 'sktt') {
    var noIsbn = spkNormalizeString_(options.noIsbn, 80);
    if (!noIsbn) return { success: false, message: 'Nomor ISBN wajib diisi untuk download SKTT.' };
    n.statusIsbn = 'QRSBN';
    n.tindakanIsbn = noIsbn;
    saveData('Naskah', H.Naskah, n, 'id');
  }
  if (type === 'isbn' && !spkNormalizeString_(options.linkPenjualan, 500)) {
    return { success: false, message: 'Link penjualan wajib diisi untuk download Permohonan ISBN.' };
  }

  var replacements = spkBuildAdminDocReplacements_(n, type, options);
  var fileName = spkAdminDocFileName_(type, n);
  var pdf = spkGenerateTemplatePDF(templateId, replacements, fileName);
  if (pdf && pdf.success) {
    pdf.updated = { isbn: spkGetNaskahIsbnText_(n), statusIsbn: n.statusIsbn || '', tindakanIsbn: n.tindakanIsbn || '' };
  }
  return pdf;
}

function spkGenerateAgreementPDF(token) {
  var n = getAllData('Naskah', H.Naskah).find(function(row) {
    return String(row.finalDataToken || '').trim() === String(token || '').trim();
  });
  if (!n) return { success: false, message: 'Data naskah tidak ditemukan.' };

  var settings = spkGetAdminSettings();
  var authors = normalizeAuthorList_(n.finalPenulisList || n.finalPenulisUtama || n.penulis || []);
  var judul = n.finalJudul || n.judul || '';
  var penulis = authors.length ? authors.join(', ') : n.penulis || '';
  var partyOneName = settings.penanggungJawab || 'Muh Kahfli';
  var partyOneTitle = settings.jabatan || 'CEO/Founder';
  var partyOneCompany = settings.atasNama || 'Publish Inc.';
  var spkNo = spkBuildAgreementNumber_(n);
  var tempFile = null;
  try {
    var doc = DocumentApp.create('Temp_SPK_PublishInc_' + new Date().getTime());
    tempFile = DriveApp.getFileById(doc.getId());
    var body = doc.getBody();
    body.setMarginTop(72).setMarginBottom(72).setMarginLeft(72).setMarginRight(72);
    spkAppendCentered_(body, 'SURAT PERJANJIAN KERJASAMA PENERBITAN', 14, true);
    spkAppendCentered_(body, 'Nomor: ' + spkNo, 11, true);
    body.appendParagraph('');
    spkAppendText_(body, 'Yang bertanda tangan di bawah ini:', 11, false);
    body.appendParagraph('');
    spkAppendPartyTable_(body, 'I.', [
      ['Nama', partyOneName],
      ['Jabatan', partyOneTitle],
      ['Bertindak atas nama', partyOneCompany]
    ]);
    spkAppendText_(body, 'Selanjutnya disebut sebagai PIHAK PERTAMA', 11, false);
    body.appendParagraph('');
    spkAppendPartyTable_(body, 'II.', [
      ['Nama', penulis],
      ['No. Identitas (KTP)', n.authorKtp || '-'],
      ['Pekerjaan/Inst.', n.authorPekerjaan || '-'],
      ['Alamat', n.authorAlamat || '-'],
      ['No. HP/WA', n.noHpPenulis || '-']
    ]);
    spkAppendText_(body, 'Selanjutnya disebut sebagai PIHAK KEDUA', 11, false);
    body.appendParagraph('');
    spkAppendText_(body, 'Kedua belah pihak sepakat mengadakan kerjasama penerbitan buku berjudul "' + judul + '" dengan pilihan ' + (n.paket || '-') + ', dengan ketentuan sebagai berikut:', 11, false);

    var pasals = Array.isArray(settings.pasals) ? settings.pasals : [];
    if (!pasals.length) {
      pasals = [{ judul: 'Ketentuan Umum', isi: 'Para pihak sepakat menjalankan kerja sama penerbitan sesuai ketentuan yang berlaku di Publish Inc.' }];
    }
    pasals.forEach(function(pasal, index) {
      body.appendParagraph('');
      spkAppendCentered_(body, 'Pasal ' + (index + 1), 11, true);
      if (pasal.judul) spkAppendCentered_(body, pasal.judul, 11, true);
      spkAppendAgreementBody_(body, pasal.isi || '-');
    });
    if (settings.penutup) {
      body.appendParagraph('');
      spkAppendAgreementBody_(body, settings.penutup);
    }
    body.appendParagraph('');
    spkAppendText_(body, 'Demikian surat perjanjian kerjasama ini disepakati dan ditandatangani oleh kedua belah pihak.', 11, false);
    body.appendParagraph('');
    body.appendParagraph('');
    var line = body.appendParagraph('____________________________________________________________');
    line.setAlignment(DocumentApp.HorizontalAlignment.CENTER);
    var approval = spkAppendCentered_(body, 'Dokumen ini telah disetujui dan ditandatangani secara elektronik (online) oleh ' + penulis + '.', 11, false);
    approval.editAsText().setItalic(true);
    doc.saveAndClose();

    var fileName = 'SPK_PublishInc_' + String(judul || 'Naskah').replace(/[^A-Za-z0-9]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 50) + '.pdf';
    var blob = tempFile.getAs('application/pdf').setName(fileName);
    return {
      success: true,
      fileName: fileName,
...
function downloadSpkPdf(token) {
  return spkGenerateAgreementPDF(token);
}

function spkBuildAgreementNumber_(n) {
  var now = new Date();
  var romanMonths = ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII'];
  var raw = String(n.trackingCode || n.id || n.finalDataToken || '').replace(/\D/g, '');
  var seq = (raw ? raw.slice(-3) : '1').padStart(3, '0');
  return seq + '/SPK-PI/' + romanMonths[now.getMonth()] + '/' + now.getFullYear();
}

function spkAppendCentered_(body, text, size, bold) {
  var p = body.appendParagraph(String(text || ''));
  p.setAlignment(DocumentApp.HorizontalAlignment.CENTER);
  var textEl = p.editAsText();
  textEl.setFontFamily('Times New Roman').setFontSize(size || 11).setBold(false);
  if (bold) textEl.setBold(true);
  p.setSpacingAfter(6);
  return p;
}

function spkAppendText_(body, text, size, bold) {
  var p = body.appendParagraph(String(text || ''));
  var textEl = p.editAsText();
  textEl.setFontFamily('Times New Roman').setFontSize(size || 11).setBold(false);
  if (bold) textEl.setBold(true);
  p.setLineSpacing(1.15).setSpacingAfter(8);
  return p;
}

function spkAppendPartyTable_(body, prefix, rows) {
  var tableRows = rows.map(function(row, index) {
    return [index === 0 ? prefix : '', row[0] || '', ':', row[1] || '-'];
  });
  var table = body.appendTable(tableRows);
  try { table.setBorderWidth(0); } catch (ignore) {}

  for (var r = 0; r < table.getNumRows(); r++) {
    var tableRow = table.getRow(r);
    for (var c = 0; c < tableRow.getNumCells(); c++) {
      var cell = tableRow.getCell(c);
      cell.setPaddingTop(0).setPaddingBottom(2).setPaddingLeft(0).setPaddingRight(4);
      if (c === 0) cell.setWidth(28);
      if (c === 1) cell.setWidth(138);
      if (c === 2) cell.setWidth(12);
      if (c === 3) cell.setWidth(330);
      var p = cell.getChild(0).asParagraph();
      p.setSpacingAfter(0);
      p.editAsText().setFontFamily('Times New Roman').setFontSize(11).setBold(false);
    }
  }
  spkAppendText_(body, '', 11, false);
}

function spkAppendAgreementBody_(body, text) {
  String(text || '').split(/\n{2,}/).forEach(function(block) {
    block = block.replace(/\s+$/g, '');
    if (!block) return;
    var p = body.appendParagraph(block);
    p.editAsText().setFontFamily('Times New Roman').setFontSize(11).setBold(false);
    p.setLineSpacing(1.35).setSpacingAfter(10);
    try { p.setAlignment(DocumentApp.HorizontalAlignment.JUSTIFY); } catch (ignore) { p.setAlignment(DocumentApp.HorizontalAlignment.LEFT); }
  });
}

function spkBuildAdminDocReplacements_(n, type, options) {
  options = options || {};
  var authors = normalizeAuthorList_(n.finalPenulisList || n.finalPenulisUtama || n.penulis || []);
  var editors = normalizeAuthorList_(n.finalEditorList || []);
  var judul = n.finalJudul || n.judul || '';
  var penulis = authors.length ? authors.join(', ') : n.penulis || '';
  var editor = editors.length ? editors.join(', ') : '-';
  var tanggal = Utilities.formatDate(new Date(), getDBTimezone(), 'dd MMMM yyyy');
  var isbn = type === 'loa' ? 'On Process' : spkGetNaskahIsbnText_(n);
  var nomor = spkAdminDocNumber_(type, n);
  return {
    '<<NO_SKTT>>': type === 'sktt' ? nomor : '',
    '<<NO_LOA>>': type === 'loa' ? nomor : '',
    '<<NO_PERMOHONAN>>': type === 'isbn' ? nomor : '',
    '<<JUDUL_BUKU>>': judul,
    ...
    '<<TANGGAL>>': tanggal,
    '<<TANGGAL)': tanggal,
    '<<LINK_PENJUALAN>>': spkNormalizeString_(options.linkPenjualan, 500) || 'https://publishinc.id/toko'
  };
}

function spkGetNaskahIsbnText_(n) {
  var tindakan = String(n.tindakanIsbn || '').trim();
  var status = String(n.statusIsbn || '').trim();
  if (/^97[89][\d\s-]{8,}$/i.test(tindakan)) return tindakan;
  if (/^97[89][\d\s-]{8,}$/i.test(status)) return status;
  if (status && !/menunggu/i.test(status)) return status;
  return 'On Process';
}

function spkAdminDocNumber_(type, n) {
  var prefix = { sktt: 'SKTT', loa: 'LOA', isbn: 'ISBN', keaslian: 'KEASLIAN' }[type] || 'ADM';
  var now = new Date();
  var romanMonths = ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII'];
  var code = String(n.trackingCode || n.id || n.finalDataToken || '').replace(/[^A-Za-z0-9]/g, '').slice(-6).toUpperCase() || '000000';
  return code + '/' + prefix + '-PI/' + romanMonths[now.getMonth()] + '/' + now.getFullYear();
}

function spkAdminDocFileName_(type, n) {
  var label = { sktt: 'SKTT', loa: 'LoA', keaslian: 'Keaslian', isbn: 'Permohonan_ISBN' }[type] || 'Dokumen';
  var title = String(n.finalJudul || n.judul || 'Naskah').replace(/[^A-Za-z0-9]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 50) || 'Naskah';
  return label + '_' + title + '.pdf';
}
...
function isCssWorkflowNaskah_(n, username) {
  if (!n || !isCssOwner_(n, username)) return false;
  if (String(n.cancelApprovedAt || '')) return false;
  if (String(n.statusProses || '').toLowerCase() === 'dibatalkan') return false;
  var source = String(n.source || '').trim().toLowerCase();
  if (source && source !== 'cs' && source !== 'migrasi') return false;
  var stage = effectiveWorkflowStageFromStatuses_(n);
  return ['administrasi', 'produksi', 'distribusi', 'selesai'].indexOf(stage) !== -1;
}
...
function isPicQueueApplicable_(n, stageKey) {
  ...
  var workflowStage = String((n || {}).workflowStage || inferWorkflowStage_(n)).trim().toLowerCase();
  if (stageKey === 'proofreading' && workflowStage !== 'pic_editor') return false;
  if (stageKey === 'layout' && workflowStage !== 'pic_layouter') return false;
  if (stageKey === 'proofreading') return !isNaskahProofreadingSkipped_(n) && !isPicProductionStageComplete_(n.statusEditor);
  if (stageKey === 'layout') return !isPicProductionStageComplete_(n.statusLayouter);
  return false;
}
...
function isPicNewNaskah_(n, stageKey) {
  return isPicQueueApplicable_(n, stageKey) && !isPicAssignedForStage_(n, stageKey);
}
...
function startNaskahProductionFromCss(naskahId, username) {
  ...
  for (var i = 0; i < rows.length; i++) {
    if (String(rows[i].id || '') !== String(naskahId || '')) continue;
    var n = rows[i];
    if (!isCssOwner_(n, username)) return { success: false, message: 'Naskah ini bukan tanggung jawab CCO Anda.' };
    ...
    n.statusEditor = skipProofreading ? 'Dilewati' : (n.statusEditor || 'Menunggu');
...
function completeCssWorkflowStage(naskahId, stageKey, username) {
  stageKey = trackingStageKey_(stageKey);
  var rows = getAllData('Naskah', H.Naskah);
  for (var i = 0; i < rows.length; i++) {
    if (String(rows[i].id || '') !== String(naskahId || '')) continue;
    ...
    if (!isCssOwner_(n, username)) return { success: false, message: 'Naskah ini bukan tanggung jawab CCO Anda.' };
    var workflowStage = String(n.workflowStage || inferWorkflowStage_(n)).trim().toLowerCase();
    if (stageKey === 'administrasi') return startNaskahProductionFromCss(naskahId, username);
    if (stageKey === 'produksi') {
      if (workflowStage !== 'produksi') return { success: false, message: 'Naskah belum berada di tahap produksi.' };
      n.statusProduksi = 'Done';
      setWorkflowStage_(n, 'distribusi', username);
      return saveNaskahData(n);
    }
    if (stageKey === 'distribusi') {
      if (workflowStage !== 'distribusi') return { success: false, message: 'Naskah belum berada di tahap distribusi.' };
      n.statusDistribusi = 'Done';
      n.statusProses = 'Selesai';
      setWorkflowStage_(n, 'selesai', username);
      return saveNaskahData(n);
    }
    return { success: false, message: 'Tahap CCO tidak dikenali.' };
  }
  return { success: false, message: 'Naskah tidak ditemukan.' };
}
...
function getCssAssignmentOptions() {
  ...
  var dealsById = {};
  getAllData('Deals', H.Deals).forEach(function(d) {
    if (d && d.id) dealsById[String(d.id)] = d;
  });
  var rows = ensureNaskahTracking_(getAllData('Naskah', H.Naskah)).filter(function(n) {
    var statusProses = String(n.statusProses || '').trim().toLowerCase();
    var workflowStage = String(n.workflowStage || '').trim().toLowerCase();
    var statusDistribusi = String(n.statusDistribusi || '').trim().toLowerCase();
    return !String(n.cancelApprovedAt || '') &&
      statusProses !== 'dibatalkan' &&
      workflowStage !== 'selesai' &&
      statusDistribusi !== 'selesai';
  });
  var data = users.map(function(u) {
    var byPublisher = {};
    var total = 0;
    rows.forEach(function(n) {
      var deal = n.dealId ? dealsById[String(n.dealId)] : null;
      var stage = String(n.workflowStage || '').trim().toLowerCase();
      var ownerUser = ['administrasi', 'produksi', 'distribusi'].indexOf(stage) !== -1 ? n.workflowOwnerUsername : '';
      var ownerName = ['administrasi', 'produksi', 'distribusi'].indexOf(stage) !== -1 ? n.workflowOwnerName : '';
      var nUser = String(n.assignedCssUsername || ownerUser || (deal && deal.assignedCssUsername) || '').trim().toLowerCase();
      var nName = String(n.assignedCssName || ownerName || (deal && deal.assignedCssName) || '').trim().toLowerCase();
...
function getCssUsersForDeal() {
  return getCssAssignmentOptions();
}

function assignNaskahCss(naskahId, cssUsername) {
  naskahId = String(naskahId || '').trim();
  cssUsername = String(cssUsername || '').trim();
  if (!naskahId || !cssUsername) return { success: false, message: 'Naskah dan CCO wajib dipilih.' };
  var users = getAllData('Users', H.Users);
  var cssUser = null;
  for (var i = 0; i < users.length; i++) {
  ...
  if (!cssUser) return { success: false, message: 'User CCO tidak ditemukan.' };
  var rows = getAllData('Naskah', H.Naskah);
...
function autoAssignUnassignedNaskah() {
  var cssUsers = getAllData('Users', H.Users).filter(function(u) { return normalizeRole_(u.role) === 'css'; });
  if (cssUsers.length === 0) return { success: false, message: 'Tidak ada akun CCO yang tersedia.' };
  var rows = getAllData('Naskah', H.Naskah);
  var assigned = 0;
  var skipped = 0;
  rows.forEach(function(n) {
...
function saveNaskahData(p) {
  ...
  if (!p.trackingCode) p.trackingCode = generateTrackingCode_();
  var assignmentChangedByPic = false;
  if (previous && (callerRole === 'pic_editor' || callerRole === 'pic_layouter')) {
    ...
    assignmentChangedByPic = !!(nextAssignee && nextAssignee !== previousAssignee);
  }
  ...
  if (!p.baseDeadlines && p.paket) {
    ...
    if (plan) {
      ...
      p.baseTimelineRanges = plan.baseRanges;
      p.timelineRanges = plan.actualRanges;
  ...
  refreshNaskahTimelineRanges_(p);
  ...
  if (previous) {
    if (callerRole === 'pic_editor' && assignmentChangedByPic && String(p.picEditor || '').trim()) {
      p.picEditorQueueStatus = 'assigned';
      p.statusEditor = p.statusEditor || 'Menunggu';
      setWorkflowStage_(p, 'editor', p.picEditor);
    } else if (callerRole === 'pic_layouter' && assignmentChangedByPic && String(p.picLayouter || '').trim()) {
      p.picLayouterQueueStatus = 'assigned';
      p.statusLayouter = p.statusLayouter || 'Menunggu';
      setWorkflowStage_(p, 'layouter', p.picLayouter);
    }
    if (!isWorkflowDoneStatus_(previous.statusEditor) && isWorkflowDoneStatus_(p.statusEditor)) {
      p.picLayouter = '';
      p.picLayouterQueueStatus = 'new';
      p.picLayouterQueuedAt = '';
      setWorkflowStage_(p, 'pic_layouter', '');
    }
    if (!isWorkflowDoneStatus_(previous.statusLayouter) && isWorkflowDoneStatus_(p.statusLayouter)) {
      p.statusProduksi = p.statusProduksi || 'Menunggu';
      setWorkflowStage_(p, 'produksi', p.assignedCssUsername || '');
    }
    if (!isWorkflowDoneStatus_(previous.statusProduksi) && isWorkflowDoneStatus_(p.statusProduksi)) {
      p.statusDistribusi = p.statusDistribusi || 'Menunggu';
      setWorkflowStage_(p, 'distribusi', p.assignedCssUsername || '');
    }
    if (!isWorkflowDoneStatus_(previous.statusDistribusi) && isWorkflowDoneStatus_(p.statusDistribusi)) {
      p.statusProses = 'Selesai';
      setWorkflowStage_(p, 'selesai', p.assignedCssUsername || '');
    }
  } else if (!String(p.workflowStage || '').trim()) {
    p.workflowStage = inferWorkflowStage_(p);
    p.timeline = workflowStageLabel_(p.workflowStage);
  }
  syncWorkflowStageFromStatuses_(p, p.workflowOwnerUsername || p.assignedCssUsername || '');
...
// ==========================================
// SPK PENULIS - ADMIN PERSURATAN
// ==========================================
var SPK_TOKEN_TTL_DAYS = 30;
var SPK_DATA_HEADERS = [
  'Token', 'No SPK', 'Nama', 'No. HP', 'Status', 'Waktu Dibuat', 'Waktu Disepakati',
  'Pekerjaan', 'No. KTP', 'Alamat', 'Judul Buku', 'Paket Pilihan', 'Penulis Fix',
  'Editor Fix', 'No ISBN', 'Ukuran', 'Token Kedaluwarsa'
];

function spkSpreadsheet_() {
  return SpreadsheetApp.openById(PORTAL_DATABASE_ID);
}

function spkEnsureSheet_(name, headers) {
  var ss = spkSpreadsheet_();
...
function spkDataSheet_() {
  return spkEnsureSheet_('Data_Penulis', SPK_DATA_HEADERS);
}

function spkGetAdminSettings() {
  var ss = spkSpreadsheet_();
  var result = {
    penanggungJawab: '',
    jabatan: '',
  ...
  if (sheetSettings && sheetSettings.getLastRow() > 1) {
    var settingsRows = sheetSettings.getRange(2, 1, sheetSettings.getLastRow() - 1, 2).getValues();
    var map = {};
  ...
  var sheetPasal = ss.getSheetByName('Template_Pasal');
  if (sheetPasal && sheetPasal.getLastRow() > 1) {
    result.pasals = sheetPasal
      .getRange(2, 1, sheetPasal.getLastRow() - 1, 3)
      .getValues()
      .filter(function(row) { return row[1] || row[2]; })
      .map(function(row) { return { judul: String(row[1] || ''), isi: String(row[2] || '') }; });
  }

  var sheetPaket = ss.getSheetByName('Paket_Penerbitan');
  if (sheetPaket && sheetPaket.getLastRow() > 1) {
    result.pakets = sheetPaket
      .getRange(2, 1, sheetPaket.getLastRow() - 1, 2)
      .getValues()
...
function spkSaveAdminSettings(data) {
  data = data || {};
  var ss = spkSpreadsheet_();

  var sheetSettings = spkEnsureSheet_('Pengaturan', ['Kunci', 'Nilai']);
  if (sheetSettings.getLastRow() > 1) {
    sheetSettings.getRange(2, 1, sheetSettings.getLastRow() - 1, 2).clearContent();
  }
  var settingsRows = [
    ['Penanggung Jawab', spkNormalizeString_(data.penanggungJawab, 150)],
    ['Jabatan', spkNormalizeString_(data.jabatan, 150)],
    ['Atas Nama', spkNormalizeString_(data.atasNama, 150) || 'Publish Inc.'],
    ['Penutup', spkNormalizeString_(data.penutup, 2000)]
  ];
  sheetSettings.getRange(2, 1, settingsRows.length, 2).setValues(settingsRows);

  var sheetPasal = spkEnsureSheet_('Template_Pasal', ['No', 'Judul Pasal', 'Isi Pasal']);
  if (sheetPasal.getLastRow() > 1) {
    sheetPasal.getRange(2, 1, sheetPasal.getLastRow() - 1, 3).clearContent();
  }
  var pasals = Array.isArray(data.pasals) ? data.pasals : [];
  if (pasals.length) {
    sheetPasal.getRange(2, 1, pasals.length, 3).setValues(pasals.map(function(pasal, index) {
      return [index + 1, spkNormalizeString_(pasal && pasal.judul, 200), spkNormalizeString_(pasal && pasal.isi, 3000)];
    }));
  }

  if (Array.isArray(data.pakets)) {
    var sheetPaket = spkEnsureSheet_('Paket_Penerbitan', ['No', 'Nama Paket']);
    if (sheetPaket.getLastRow() > 1) {
      sheetPaket.getRange(2, 1, sheetPaket.getLastRow() - 1, 2).clearContent();
    }
    var pakets = data.pakets.filter(Boolean);
    if (pakets.length) {
      sheetPaket.getRange(2, 1, pakets.length, 2).setValues(pakets.map(function(paket, index) {
        return [index + 1, spkNormalizeString_(paket, 200)];
      }));
    }
  }
...
function spkGenerateToken(nama, hp, isbn, ukuran) {
  var cleanNama = spkNormalizeString_(nama, 150);
  var cleanHp = spkNormalizePhone_(hp);
  ...
  lock.waitLock(30000);
  try {
    var sheet = spkDataSheet_();
    var token = spkGenerateUniqueToken_();
    var noSpk = spkGenerateNoSPK_();
    var createdAt = new Date();
    var expiresAt = new Date(createdAt.getTime() + SPK_TOKEN_TTL_DAYS * 24 * 60 * 60 * 1000);
    ...
    sheet.appendRow([
      ...
      '-',
      '-',
      spkNormalizeString_(isbn, 80) || 'On Process',
      spkNormalizeString_(ukuran, 80) || 'A5',
      expiresAt
    ]);
    SpreadsheetApp.flush();
    return spkGetTokensList();
  } finally {
    lock.releaseLock();
  }
}

function spkGetTokensList() {
  var sheet = spkDataSheet_();
  var data = sheet.getDataRange().getValues();
  if (data.length <= 1) return [];
  ...
  for (var i = data.length - 1; i > 0; i--) {
    var row = data[i];
    tokens.push({
      token: spkCellString_(row[0]),
      noSpk: spkCellString_(row[1]),
      nama: spkCellString_(row[2]),
      hp: spkCellString_(row[3]),
      status: spkCellString_(row[4]) || 'PENDING',
      createdAt: spkCellString_(row[5]),
      completedAt: spkCellString_(row[6]),
      authorData: {
        pekerjaan: spkCellString_(row[7]),
        ktp: spkCellString_(row[8]),
        alamat: spkCellString_(row[9]),
        judulBuku: spkCellString_(row[10]),
        paket: spkCellString_(row[11]),
        penulisFix: spkCellString_(row[12]),
        editorFix: spkCellString_(row[13]),
        isbn: spkCellString_(row[14]) || 'On Process',
        ukuran: spkCellString_(row[15]) || 'A5'
      },
      expiresAt: spkCellString_(row[16]),
      isExpired: spkIsTokenExpired_(row[16])
    });
  }
  return tokens;
}

function spkUpdateNaskahFix(token, data) {
  var sheet = spkDataSheet_();
  var found = spkFindTokenRow_(String(token || '').trim().toUpperCase());
  if (!found) throw new Error('Token tidak valid.');
  data = data || {};

  if (data.judulBuku !== undefined) sheet.getRange(found.rowIndex, 11).setValue(spkNormalizeString_(data.judulBuku, 250) || '-');
  if (data.penulisFix !== undefined) sheet.getRange(found.rowIndex, 13).setValue(spkNormalizeString_(data.penulisFix, 250) || '-');
  if (data.editorFix !== undefined) sheet.getRange(found.rowIndex, 14).setValue(spkNormalizeString_(data.editorFix, 250) || '-');
  if (data.isbn !== undefined) sheet.getRange(found.rowIndex, 15).setValue(spkNormalizeString_(data.isbn, 80) || 'On Process');
  if (data.ukuran !== undefined) sheet.getRange(found.rowIndex, 16).setValue(spkNormalizeString_(data.ukuran, 80) || 'A5');
  SpreadsheetApp.flush();
  return spkGetTokensList();
}

function spkGenerateTemplatePDF(docId, replacements, outputName) {
  var tempFile = null;
  try {
    var templateDoc = DriveApp.getFileById(docId);
    if (templateDoc.getMimeType() !== MimeType.GOOGLE_DOCS) {
      return { success: false, error: 'File template bukan Google Docs.' };
    }
    tempFile = templateDoc.makeCopy('Temp_Document_SPK_' + new Date().getTime());
    var tempDoc = DocumentApp.openById(tempFile.getId());
    var targets = [tempDoc.getBody()];
    ...
    Object.keys(replacements).forEach(function(key) {
      var pattern = spkEscapeReplacePattern_(key);
      var value = String(replacements[key] || '').substring(0, 1000);
      targets.forEach(function(target) {
        target.replaceText(pattern, value);
      });
    });
    tempDoc.saveAndClose();
    var blob = tempFile.getAs('application/pdf').setName(outputName || 'Dokumen_SPK.pdf');
    return {
      success: true,
...
function spkEscapeReplacePattern_(text) {
  return String(text || '').replace(/[\\^$.*+?()[\]{}|]/g, '\\$&');
}

function spkGenerateNoSPK_() {
  var props = PropertiesService.getScriptProperties();
  var now = new Date();
  var year = now.getFullYear();
  var romanMonths = ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII'];
  var month = romanMonths[now.getMonth()];
  var key = 'SPK_COUNTER_' + year + '_' + month;
  var current = Number(props.getProperty(key) || spkGetMaxSpkNumberForPeriod_(month, year));
  var nextNum = current + 1;
  props.setProperty(key, String(nextNum));
  return String(nextNum).padStart(3, '0') + '/SPK-PI/' + month + '/' + year;
}

function spkGetMaxSpkNumberForPeriod_(month, year) {
  var sheet = spkDataSheet_();
  var lastRow = sheet.getLastRow();
  if (lastRow <= 1) return 0;
  var values = sheet.getRange(2, 2, lastRow - 1, 1).getValues();
  var pattern = new RegExp('^(\\d+)/SPK-PI/' + month + '/' + year + '$');
...
function spkGenerateUniqueToken_() {
  var existing = new Set(spkDataSheet_().getDataRange().getValues().slice(1).map(function(row) {
...
function spkFindTokenRow_(token) {
  var sheet = spkDataSheet_();
  var data = sheet.getDataRange().getValues();
  for (var i = 1; i < data.length; i++) {
...
function spkIsTokenExpired_(expiresAt) {
...
function spkNormalizeString_(value, maxLength) {
  return String(value || '').replace(/[\u0000-\u001F\u007F]/g, ' ').trim().substring(0, maxLength || 500);
}

function spkNormalizePhone_(value) {
  return String(value || '').replace(/[^\d+]/g, '').substring(0, 20);
}

function spkCellString_(value) {
  if (value instanceof Date) {
    return Utilities.formatDate(value, getDBTimezone(), 'yyyy-MM-dd HH:mm');
  }
